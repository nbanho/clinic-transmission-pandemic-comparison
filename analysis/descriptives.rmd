...
custom_boxplot <- function(dat) {
  ...
  dat %>%
    ...
    geom_jitter(width = .2, height = 0, alpha = .15) +
...
```{r summary-pvalue}
p_value_overall <- function(dat, method = "gaussian") {
...
pvals_overall <- data.frame(
  ...
  pvalue = c(
    ...
    p_value_overall(df %>% mutate(y = diagnosed), method = "poisson"),
    p_value_overall(df %>% mutate(y = registered), method = "poisson"),
