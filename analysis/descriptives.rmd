...
custom_boxplot <- function(dat) {
  p_value_1 <- ggpubr::compare_means(
    y ~ year,
    data = dplyr::filter(dat, daytime == "Morning"), method = "wilcox.test"
  )$p.adj
  p_value_2 <- ggpubr::compare_means(
    y ~ year,
    data = dplyr::filter(dat, daytime == "Afternoon"), method = "wilcox.test"
  )$p.adj
...
well_riley_pl <- well_riley %>%
  ...
  labs(
    ...
    title = "C | Modelled transmission risk",
  ) +
  scale_y_continuous(
    limits = c(0, .05),
...
mol_pl <- df %>%
  ...
  labs(
    ...
    title = "D | Mtb concentration in bioaerosols",
  ) +
  scale_y_continuous(
    limits = c(0, 10),
...
descriptive_pl <- track_pl + aer_pl + well_riley_pl + mol_pl +
...
ggsave(
  plot = descriptive_pl,
  filename = "results/descriptives.tiff",
  width = 16 / cm(1), height = 20 / cm(1)
)
```
...
**Table**: Sensitivity analysis for the modelled risk of Mtb transmission.
